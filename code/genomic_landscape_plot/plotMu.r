...
geom_point(size=lineWidth,color=ifelse(dataFst1Subset$WEIR_AND_COCKERHAM_FST>=0.8,"red","black")) +
facet_wrap(~ chr,nrow=1) +
...
geom_point(size=lineWidth,color=ifelse(dataFst2Subset$WEIR_AND_COCKERHAM_FST>=0.8,"red","black")) +
facet_wrap(~ chr,nrow=1) +
...
geom_point(size=lineWidth,color=ifelse(dataFst3Subset$WEIR_AND_COCKERHAM_FST>=0.8,"red","black")) +
facet_wrap(~ chr,nrow=1) +
...
plotPanel <- plot_grid(g0,g5.3,g5.2,g5.1,align="v",nrow=4,rel_heights=c(0.8,1,1,1))

write("--------Export to a PNG file", stdout())
## Original figure
#png(args[10],2000,1300)
## New figure
png(args[10],3000,800)
grid.draw(plotPanel)
dev.off()
write("--------Done",stdout())
